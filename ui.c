...
char* gogui_analyze_commands(void)
{
    return "param/           General Parameters/param_general\n"
           ...
           "gfx/gfx   Priors/priors\n"
           "gfx/gfx   Children/children\n"
...
char* gtp_children(Position* pos, TreeNode* tree)
{
    char str[12], str2[20];
    TreeNode* n;
...
char* gtp_priors(Position* pos, TreeNode* tree)
{
    char str[12], str2[20];
    double dvmax;
    int   vmax = 0;
...
// use the live gfx capability of gogui to do animation during the search
{
    ...
    else if (strcmp(Live_gfx, "priors") == 0)
        ret = gtp_priors(pos, tree);
    else if (strcmp(Live_gfx, "children") == 0)
        ret = gtp_children(pos, tree);
...
// Loop that process the gtp commands send by the controller
{
    ...
    for(;;) {
        ...
        if (strcmp(command, "play")==0)
            ...
            ret = gtp_visit_count(pos, tree);
        else if (strcmp(command, "priors") == 0)
            ret = gtp_priors(pos, tree);
...
int hobot_console(int argc, char *argv[]) 
{
    char *command;
    double score;
    ...
    saved_nodes = 0;
    // Setup log
    flog = fopen("hobot.log", "w");
    setbuf(flog, NULL);                // guarantees that log is unbuffered
    log_fmt_s('I',"hobot Version %s", version);
    ...
    // Run time checks that hobot requirements are fulfilled
    if (sizeof(int) < 4)
        fatal_error("hobot needs size of int >= 4 bytes (see hobot.log)");
    if (sizeof(long long) < 8)  // should never happen by C99 standard
        fatal_error("hobot needs size of long long >= 8 bytes(see hobot.log)");

    // Initialization of all global data
    make_pat3set();
    already_suggested = hobot_calloc(1, sizeof(Mark));
    board_init();
    int      *amaf_map=hobot_calloc(BOARDSIZE, sizeof(int)); 
    int      *owner_map=hobot_calloc(BOARDSIZE, sizeof(int));
    int      *score_count=hobot_calloc(2*N*N+1, sizeof(int));
