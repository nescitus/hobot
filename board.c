...
{
    mark1 = hobot_calloc(1, sizeof(Mark));
    mark2 = hobot_calloc(1, sizeof(Mark));
}
...
//                                                        . #           X X X
{
    Color eyecolor = is_eyeish(pos, pt);
    Color falsecolor;
    ...
    FORALL_DIAGONAL_NEIGHBORS(pos, pt, k, d) {
        if (point_color(pos, d) == OUT) 
            at_edge = 1;
        else if (point_color(pos, d) == falsecolor) 
            false_count += 1;
    }
...
// TODO: change char* to int denoting error type
