...
// Analyse the subcommands of the gtp debug command
{
    ...
    else if (strcmp(command, "gen_playout") == 0) 
    {
        ...
        if (suggestion != NULL) {
            ...
            make_list_last_moves_neighbors(pos, last_moves_neighbors, 2);
    ...
    else if (strcmp(command, "atari") == 0) 
    {
        ...
        if (str == NULL) ret = "Error missing point";
            ...
            if (point_color(pos, pt) == EMPTY) {
                ret = "Error given point is empty";
                goto finished;
            }
    ...
    else if (strcmp(command, "min_lib") == 0) 
    {
        ...
        Point pt = parse_coord(str, 0);
        if (point_color(pos, pt) != EMPTY) {
            ret = "Error given point not empty";
            goto finished;
        }
        ...
        if (min_libs < 6) {
            if (min_libs == 0) ret = "wtf: 0";
            if (min_libs == 1) ret = "interesting: 1";
            if (min_libs == 2) ret = "interesting: 2";
            if (min_libs == 3) ret = "interesting: 3";
            if (min_libs == 4) ret = "interesting: 4";
            if (min_libs == 5) ret = "interesting: 5";
        }
        else
            ret = "boring";
    }
    */
        ...
        ret = slist_str_as_point(moves); // list of atari escapes
        int l = (int)strlen(ret);

        // is_atari, 2 libs, other side to move, one killing move, 
        // list not empty, line_height > 2 = laddered/netted
        
        // is_atari, 1 lib, side to move, list empty = caught
        ...
        else if (strcmp(str, "last4") == 0)
            str_coord(board_last4(pos), buf);
