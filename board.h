...
// implementation easier).  Coordinates are just indices in this string.
    ...
    Point last, last2, last3, last4; // position of the last move and the move before 
...
void   make_list_last_moves_neighbors(Position *pos, Slist points, int nb);
...
void*  hobot_calloc(size_t nmemb, size_t size);
void*  hobot_malloc(size_t size);
Point  parse_coord(char *s, int rot);
...
__INLINE__ Point board_last4(Position* pos) { return pos->last4; }
...
__INLINE__ void  board_set_last4(Position* pos, Point pt) { pos->last4 = pt; }
...
__INLINE__ int   point_is_color(Position *pos, Point pt)  
...
// Assertion check
//#ifdef NDEBUG
  #define hobot_assert(pos, condition)
/**#else
  #define hobot_assert(pos, condition)                                        \
    if (!condition) {                                                         \
...
// These two representations are necessary for performance.
// 
