...
int   PRIOR_SELFATARI    = 24;   // negative prior
int   PRIOR_CAPTURE_ONE  = 70;
int   PRIOR_CAPTURE_TWO  = 90;
int   PRIOR_CAPTURE_MANY = 140;
int   PRIOR_ATARI        = 5;
int   PRIOR_DBL_ATARI    = 20;
int   PRIOR_PAT3         = 10;
...
double PROB_HEURISTIC_LARGE_PAT = 0.10;
...
double PROB_UNIFORM_PLAYOUT = 0.15;
...
// Implement gtp command that list or modify parameters of the playout policy
{
    ...
    if (param == NULL) {    // List current values
        ...
        PRINT_KEY_VALUE(PROB_HEURISTIC_LARGE_PAT, %.2f);
        PRINT_KEY_VALUE(PROB_SSAREJECT, %.2f);
        PRINT_KEY_VALUE(PROB_RSAREJECT, %.2f);
        PRINT_KEY_VALUE(PROB_UNIFORM_PLAYOUT, %.2f);
    }
    ...
    else if (strcmp(param, "PROB_HEURISTIC_LARGE_PAT") == 0)
        READ_VALUE(PROB_HEURISTIC_LARGE_PAT, %lf)
    ...
    else if (strcmp(param, "PROB_UNIFORM_PLAYOUT") == 0)
        READ_VALUE(PROB_UNIFORM_PLAYOUT, %lf)
    else {
...
// Implement gtp command that list or modify parameters of the tree policy
{
    ...
    if (param == NULL) {    // List current values
        ...
        PRINT_KEY_VALUE(PRIOR_CAPTURE_TWO, %d);
        ...
        READ_VALUE(N_SIMS, %d)
    else if (strcmp(param, "MAX_TIME") == 0)
        READ_VALUE(MAX_TIME, %d)
    ...
    else if (strcmp(param, "PRIOR_CAPTURE_TWO") == 0)
        READ_VALUE(PRIOR_CAPTURE_TWO, %d)
...
// Implement gtp command that list or modify general parameters
{
    ...
    if (param == NULL) {    // List current values
        ...
        PRINT_LIST_KEY_VALUE("[list/None/best_moves/owner_map/"
                                   "principal_variation/visit_count/priors/children]",
                                   Live_gfx, %s);
