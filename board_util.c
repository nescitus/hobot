...
Point parse_coord(char* s, int rot)
{
    ...
    if (isalpha(str[0]) && isdigit(str[1])
        ...
        // 7 rotations combine these possible steps:
        // switching x and y, mirroring x and mirroring y
        ...
        if (rot == 2 || rot == 4 || rot == 5 || rot == 7) {
            x = 20 - x;
        }

        if (rot == 3 || rot == 4 || rot == 6 || rot == 7) {
            y = 20 - y;
        }
